...
return d.opacity;
	...
	thisModule.initialize = function (lastDigitsIP, nTimeSlices, gridID, gridColorID, heatmapDivID, show_floors, showStatus, transParams, crtsMetricsId) {
		...
		socket.on('crtsPerformanceMetrics', function(data) {
			if(thisModule.connected) {
				document.getElementById(crtsMetricsId).innerHTML = "<tr><td> Packet error rate: </td> <td>" + data.toString() + "</td></tr>";
			}
		});
		...
		socket.on('initial_params', function(data) {
			console.log(JSON.stringify(data));
                ...
                var new_json ='';
                var obj='';
                var data_json = '[{"bandwidth": 2000, "frequency": 7500000, "node": 0}]';
		socket.on('spectrumUpdate', function(data) {
			console.log(JSON.stringify(data));
                       ...
                       //data_json.push(JSON.stringify(data));
                        ...
                        /*var txtFile = "c:/json.txt";
                        var file = new File(txtFile);
                        file.open("w"); // open file with write access
                        file.write(JSON.stringify(new_json));
                        file.close();*/
        ...
        thisModule.launchScoreboard = function()
          {
          socket.emit('launchScoreboard',18);   
          }
